...
class GazelleCrawler:
    ...
    async def test_auth(self) -> SiteAuthCheck:
        ...
        try:
            ...
            if not isinstance(response, dict) or not _text(response.get("username")):
                return SiteAuthCheck(False, "站点登录状态失效，错误信息：未返回用户信息。")
        except GazelleAPIError as exc:
            return SiteAuthCheck(False, f"站点登录状态失效，错误信息：{exc.message}")
        except Exception as exc:  # noqa: BLE001
            return SiteAuthCheck(False, f"站点连接测试失败：{exc}")
        return SiteAuthCheck(True, f"站点登录状态有效，状态 {payload['status']}。")
    ...
    async def _get_json(self, path: str, params: dict[str, str]) -> dict[str, Any]:
        ...
        if not isinstance(payload, dict):
            raise RuntimeError(f"{self.name} API 响应格式无效。")
        status = _text(payload.get("status")) or "未知"
        if status != "success":
            raise GazelleAPIError(_api_error(payload))
        return payload
    ...
    def _search_results(self, raw_group: object) -> tuple[SearchResult, ...]:
        ...
        for torrent in torrents:
            ...
            release_type = _text(raw_group.get("releaseType"))
            ...
            results.append(
                SearchResult(
                    ...
                    metadata={
                        "adapter": "gazelle",
                        "type": " / ".join(part for part in ("music", release_type) if part),
                        "category": release_type or "Music",
                        "tags": _tags(raw_group.get("tags")),
                        "artist": artist,
                        "album": group_name,
...
class GazelleAPIError(RuntimeError):
...
def _tags(value: object) -> list[str]:
    if isinstance(value, str):
        ...
        return []
    return list(dict.fromkeys(tag for item in values if (tag := item.strip())))
