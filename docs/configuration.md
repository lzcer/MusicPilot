...
| 目录 | 用途 | Docker 示例 |
| --- | --- | --- |
| 数据目录 | 保存 SQLite 数据库、导入导出文件等运行数据；使用 PostgreSQL 时仍可保存导出文件等本地数据 | `./data:/data` |
...
| `MP_DATABASE_URL` | `sqlite+aiosqlite:///./data/musicpilot.db` | 数据库连接串。快速试用可用 SQLite；长期运行和正式部署推荐使用 PostgreSQL。 |
...
Docker Compose 中至少应修改管理员密码和会话密钥。快速试用可以先使用 SQLite：
...
MusicPilot 支持 SQLite 和 PostgreSQL。SQLite 部署简单，适合本地开发、快速试用或数据量很小的单机环境；长期运行和正式部署推荐使用 PostgreSQL。
...
已有 PostgreSQL 时，把 `MP_DATABASE_URL` 改成：

```yaml
MP_DATABASE_URL: postgresql+asyncpg://musicpilot:change-this-password@postgres:5432/musicpilot
```

如果希望把 PostgreSQL 一起放进 Docker Compose，可以增加一个 `postgres` 服务：
...
生产环境不要复用示例密码。首次启动前应确认 PostgreSQL 数据目录已经持久化，并把数据库备份纳入日常备份策略。
...
| 自动分类 | 开 / 关 | 根据艺术家或专辑建立分类目录。 |
| 分类方式 | 艺术家、专辑 | 开启自动分类后生效。 |
