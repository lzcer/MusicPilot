...
| `TZ` | `Asia/Shanghai` | 容器时区。默认使用上海时区；可按部署所在地修改。 |
...
刮削用于在下载完成、源目录新增音乐或手动整理文件时补齐元数据、歌词、标签和目标路径。

| 字段 | 可选值 | 说明 |
| --- | --- | --- |
| 开启刮削 | 开 / 关 | 关闭后不会自动执行刮削流程。 |
| 自动整理 | 下载器监听、目录监听 | 下载器监听在下载完成后继续整理；目录监听让下载任务到完成即结束，并由源目录新增音乐触发整理。默认使用下载器监听。 |
| 通知延迟 | 秒，默认 10，最短 1 | 仅选择目录监听时显示；在设定时间内持续有文件入库会重新计时，并合并为一条通知。 |
| 刮削类型 | 源文件、映射文件、复制文件 | 控制处理后的文件来源和落点。 |
| 源文件目录 | 目录选择 | 手动整理和目录监听使用的源目录。候选目录来自 MusicPilot 运行环境可见的文件系统。 |
| 映射目录 / 复制目录 | 目录选择 | 选择“映射文件”或“复制文件”时填写。候选目录来自 MusicPilot 运行环境可见的文件系统。 |
| 缺失时尝试刮削 | 专辑、艺术家、歌词、封面 | 当这些字段缺失时尝试在线补齐；封面按音频文件是否包含内嵌封面判断。 |
| 缺失则判定失败 | 专辑、艺术家、歌词、封面 | 这些字段仍缺失时，本次整理视为失败；必需封面会在标签写入后再次检查。 |
| 歌曲版本控制 | 开 / 关，默认关闭 | 控制刮削、重复文件和“已在库”判断是否严格区分歌曲版本。 |
...
目录监听启动或源目录变化时只建立当前文件基线，不整理已经存在的文件。每个新增音乐文件写入稳定后独立创建“目录捕获”任务；该任务只按下载明细的完整相对路径关联，路径不一致、无唯一匹配时不使用文件名兜底，而是进入现有手动刮削链。匹配成功时复用下载明细的预刮削结果并继续原下载任务链，后续文件整理和媒体库刷新仍由原任务链自动触发。目录监听不区分下载文件和外部文件，也不执行批次合并。普通修改不会重复触发，删除后重新创建则视为新的文件。映射和复制模式也始终监听源文件目录，再按刮削类型写入目标目录。

目录捕获进入手动刮削链后，只有文件整理成功且媒体服务器接受刷新请求时才会进入通知聚合窗口；媒体库刷新仍按文件分别执行。窗口默认 10 秒，每次有新文件入库都会重新计时，到期后向已启用“媒体库通知”的 Telegram 渠道发送一条合并通知。匹配到下载任务的文件继续使用原下载任务通知，不进入该聚合窗口；普通手动整理也不会新增通知。切换回下载器监听不会丢弃已经等待发送的通知，正常关闭服务时会立即发送剩余内容。

#### 2.1.2.1. 目录监听方式

首次选择目录监听并保存时，Linux 环境会先识别源目录所在的文件系统。NFS、CIFS/SMB 和 SSHFS 等已知网络文件系统无法保证传递其他客户端的文件事件，会直接说明原因并询问是否改用轮询监听；其他文件系统则会在实际源目录中创建一个非音乐探测文件，确认该目录能够及时传递原生文件事件。探测失败时不会自动降级。源目录不存在或不可访问时，原生监听和轮询监听都不可用。

原生监听运行期间不执行 MusicPilot 自己的周期性全目录扫描，只依赖文件系统事件。系统每小时执行一次轻量健康探测；同一次探测连续两次失败后会停止监听，不会自动切换为轮询，并通过所有已启用的通知渠道发送一次告警。刮削设置的“自动整理”旁会显示红色感叹号，点击后可以重新检测原生监听、明确改用轮询、切回下载器监听或取消。

轮询监听由 `watchfiles` 按配置间隔递归检查源目录，默认间隔为 60 秒，最短为 30 秒，MusicPilot 不会再叠加自己的 60 秒快照。目录文件越多、间隔越短，CPU、磁盘和 NAS 网络 I/O 开销越高；新文件发现最多还会延迟一个轮询周期，之后仍需等待文件稳定检查。当前使用轮询时，页面会显示轮询间隔和“重新检测原生监听”操作。

Windows、Linux、macOS 和常见 ARM 平台均可使用目录监听。Linux 下识别到已知网络文件系统时会直接询问是否使用轮询；Docker 挂载目录或无法识别的特殊文件系统仍以实际源目录探测结果为准。轮询始终需要用户明确确认。

“歌曲版本控制”识别普通版、Live、Remix、Acoustic、Instrumental、Karaoke、Demo 以及 feat/合唱参与艺人：

1. 刮削：开启后只接受版本及合作艺人一致的候选；关闭时版本仅参与候选排序。
2. 重复判断：开启后不同歌曲版本可以分别入库；关闭时沿用现有规则。
3. 已在库：开启后只在音乐库存在相同版本时显示；关闭时沿用现有规则。
4. 重新刮削：同一源文件始终替换原结果，不会保留两个版本。

选择“映射文件”且无需写入标签时，MusicPilot 会优先创建硬链接。源文件目录和映射目录必须位于同一个 `/media` 挂载点，并且宿主机目录不能跨文件系统或 Btrfs 子卷；否则会自动改用复制并占用额外磁盘空间。

目录选择器只能浏览 MusicPilot 进程或容器已经挂载并有权访问的目录，不会自动创建目录或 Docker 挂载。Linux 和 Docker 环境从 `/` 开始选择；Windows 直接运行时先选择磁盘，再逐级进入目录。
...
### 4.6. 刮削结果缺少专辑、艺术家、歌词或封面

检查“缺失时尝试刮削”和“缺失则判定失败”。如果某个字段被设置为“缺失则判定失败”，在线源也没有补齐该字段时，本次整理会失败并记录原因。封面字段以最终音频文件中的内嵌封面为准，封面下载或写入失败也会被识别为缺失。MP3 和 FLAC 支持写入 JPEG、PNG 与 WebP 封面；MP4 仅写入 JPEG 或 PNG。
